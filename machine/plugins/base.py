...
############### my methods #######################
    def set_channel_webapi(self, channel, text):
        return self._client.set_channel_topic_webapi(channel, text)

    def read_channel_webapi(self, channel):
        return self._client.read_channel_topic_webapi(channel)

    def set_group_webapi(self, channel, text):
        return self._client.set_group_topic_webapi(channel, text)
...
############### my methods #######################
    ...
    def getChannelType(self, channel):
        if re.compile(r'^G.+').match(channel):
            return 'group'
        elif re.compile(r'^C.+').match(channel):
            return 'channel'

    def setTopic(self, text, channel):
        get_channel = channel
        text = text
